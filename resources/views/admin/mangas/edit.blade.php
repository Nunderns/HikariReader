...
@section('title', 'Editar Mangá: ' . $manga->title)

@section('header', 'Editar Mangá: ' . $manga->title)

@section('content')
<div class="bg-white shadow rounded-lg overflow-hidden">
    <form action="{{ route('admin.mangas.update', $manga->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="px-4 py-5 sm:p-6">
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                ...
                <div class="sm:col-span-4">
                    ...
                    <input type="text" name="english_title" id="english_title" value="{{ old('english_title', $manga->english_title) }}"
                           class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    @error('english_title')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="sm:col-span-4">
                    ...
                    <input type="text" name="artist" id="artist" value="{{ old('artist', $manga->artist) }}"
                           class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    @error('artist')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="sm:col-span-6">
                    ...
                    <div class="mt-1 flex items-center">
                        <span class="inline-block h-40 w-32 rounded overflow-hidden bg-gray-100">
                            <img id="cover-preview" src="{{ $manga->cover_url ?? 'https://via.placeholder.com/200x300?text=Sem+Imagem' }}" alt="Pré-visualização da capa" class="h-full w-full object-cover">
                        </span>
                        <input type="file" name="cover" id="cover" accept="image/*" class="ml-5 block text-sm text-gray-500" onchange="previewCover(this)">
                    </div>
                    <p class="mt-2 text-sm text-gray-500">Deixe em branco para manter a imagem atual. Formatos: JPG, PNG, GIF (máx. 2MB)</p>
                    @error('cover')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="sm:col-span-6">
                    ...
                    <div class="mt-1">
                        <textarea id="description" name="description" rows="6" required
                                  class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-300 rounded-md">{{ old('description', $manga->description) }}</textarea>
                    </div>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="sm:col-span-3">
                    ...
                    <select id="status" name="status" required
                        ...
                        <option value="completed" {{ old('status', $manga->status) == 'completed' ? 'selected' : '' }}>Completo</option>
                        <option value="hiatus" {{ old('status', $manga->status) == 'hiatus' ? 'selected' : '' }}>Em hiato</option>
                        <option value="cancelled" {{ old('status', $manga->status) == 'cancelled' ? 'selected' : '' }}>Cancelado</option>
                        <option value="not_yet_published" {{ old('status', $manga->status) == 'not_yet_published' ? 'selected' : '' }}>Não publicado</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="sm:col-span-3">
                    ...
                    <input type="text" name="demographic" id="demographic" value="{{ old('demographic', $manga->demographic) }}"
                           class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                ...
                <div class="sm:col-span-3">
                    ...
                    <input type="text" name="serialization" id="serialization" value="{{ old('serialization', $manga->serialization) }}"
                           class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    @error('serialization')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="sm:col-span-3">
                    ...
                    <input type="date" name="published_date" id="published_date" 
                           value="{{ old('published_date', $manga->published_date ? $manga->published_date->format('Y-m-d') : '') }}"
                           class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                ...
                <div class="sm:col-span-6">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
                        @php
                            ...
                            $currentGenres = json_decode($manga->genres, true) ?? [];
                            $selectedGenres = old('genres', $currentGenres);
                        @endphp
                        
                        @foreach($genres as $genre)
                            <div class="flex items-center">
                                <input id="genre-{{ Str::slug($genre) }}" name="genres[]" type="checkbox" value="{{ $genre }}"
                                       {{ is_array($selectedGenres) && in_array($genre, $selectedGenres) ? 'checked' : '' }}
                ...
                <div class="sm:col-span-6">
                    ...
                    @php
                        $currentThemes = json_decode($manga->themes, true);
                        $themesValue = is_array($currentThemes) ? implode(', ', $currentThemes) : '';
                    @endphp
                    <input type="text" name="themes" id="themes" value="{{ old('themes', $themesValue) }}"
                           class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                ...
                <div class="sm:col-span-6">
                    ...
                    @php
                        $currentAltTitles = json_decode($manga->alt_titles, true);
                        $altTitlesValue = is_array($currentAltTitles) ? implode("\n", $currentAltTitles) : '';
                    @endphp
                    <textarea id="alt_titles" name="alt_titles" rows="3"
                              class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border border-gray-300 rounded-md">{{ old('alt_titles', $altTitlesValue) }}</textarea>
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="is_adult" name="is_adult" type="checkbox" value="1" 
                                   {{ old('is_adult', $manga->is_adult) ? 'checked' : '' }}
                ...
                <div class="sm:col-span-6">
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="is_suggestive" name="is_suggestive" type="checkbox" value="1" 
                                   {{ old('is_suggestive', $manga->is_suggestive) ? 'checked' : '' }}
                                   class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                        </div>
                        <div class="ml-3 text-sm">
